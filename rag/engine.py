...
class Document(BaseModel):
    ...
    source: str = Field(
        ...,
        description="Data source: wikipedia | wikivoyage | nominatim",
    )
    content: str = Field(..., description="Text content (chunked)")
...
class RAGResult(BaseModel):
    ...
    documents: List[Document] = Field(default_factory=list, description="Retrieved relevant documents")
    ai_notes: str = Field(default="", description="AI notes generated by LLM")
    image_urls: List[str] = Field(default_factory=list, max_length=5, description="Attraction image URLs (up to 5)")
    rating: float = Field(default=0.0, ge=0.0, le=5.0, description="Attraction rating")
    review_count: int = Field(default=0, ge=0, description="Number of reviews")
    category: str = Field(default="Attraction", description="Attraction type")
...
class RAGEngine:
    ...
    result = await engine.query("Kinkaku-ji", city="Kyoto", country="Japan")
    """
    ...
    async def generate_ai_notes(
        ...
        if rag_failed or not documents:
            ...
            user_prompt = (
                f"Write practical travel notes for \"{attraction_name}\" "
                f"(located in {city}, {country}). "
                "Include the best time to visit, important tips, and local insights. "
                "Please answer in English, approximately 150 words."
            )
            suffix = "\n\n(Source: AI generated)"
        else:
            ...
            user_prompt = (
                ...
                f"Based on the above material, write practical travel notes for \"{attraction_name}\" "
                f"(located in {city}, {country}). "
                "Include the best time to visit, important tips, and local insights. "
                "Please answer in English, approximately 150 words."
            )
        ...
        try:
            ...
            return f"(AI notes generation failed: {exc})(Source: AI generated)"
